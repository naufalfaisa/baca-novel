<x-app-layout>
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            <x-back-button :href="route('author.dashboard')" class="!mb-0" />
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                ...
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('author.chapters.create', $novel) }}" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1.5 rounded-md font-medium inline-flex items-center shrink-0 transition-colors">
                        <i class="fa-solid fa-plus mr-1"></i>Add Chapter
                    </a>
                    <a href="{{ route('author.novels.edit', $novel) }}" class="bg-gray-200 hover:bg-gray-300 px-3 py-1.5 rounded-md font-medium inline-flex items-center shrink-0 transition-colors">
                        <i class="fa-solid fa-pen mr-1"></i>Edit Novel
                    </a>
                    <form method="POST" action="{{ route('author.novels.destroy', $novel) }}" onsubmit="return confirm('Delete this novel and all its chapters?')">
                        @csrf
                        @method('DELETE')
                        ...
                        @foreach ($chapters as $chapter)
                            <li class="flex items-center justify-between px-4 py-3 group hover:bg-gray-50 transition-colors">
                                ...
                                <div class="flex items-center gap-4 shrink-0">
                                    <a href="{{ route('author.chapters.edit', [$novel, $chapter]) }}" class="text-gray-600 font-semibold text-sm hover:underline transition-colors">
                                        Edit
                                    </a>
                                    <form method="POST" action="{{ route('author.chapters.destroy', [$novel, $chapter]) }}" onsubmit="return confirm('Delete this chapter?')" class="flex items-center">
                                        @csrf
                                        @method('DELETE')
